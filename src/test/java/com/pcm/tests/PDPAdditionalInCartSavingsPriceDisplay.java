...
public class PDPAdditionalInCartSavingsPriceDisplay {
	...
	@Test
	public void PDP_Additional_InCart_Savings_Price_Display() throws Exception{
		...
		try{
			...
			for(int i = 1; i <= rowCtr; i++){
				...
				//Verify the Click Here for Lowest Price price link
				testStatus = verifyXPath.isfound(Config.driver, pr.getProperty("PDP_LINK_CLICKHERELOWERPRICE_XPATH"));
				StatusLog.printlnPassedResultTrue(Config.driver,"[TESTCASE] Click Here for Lowest Price link is display in PDP",testStatus);
				
				//Verify Why don't we show price link
				testStatus = verifyXPath.isfound(Config.driver, pr.getProperty("PDP_LINK_WHYDONTWESHOWLOWERPRICE__XPATH"));
