...
public class VerifyOrderNewCustomerSubmitOrder {
	...
	@Test
	public void Verify_Order_New_Customer_Submit_Order() throws Exception{
		...
		try{
			...
			for(int i = 1; i <= rowCount; i++){
				...
				cardNo = TableContainer.getCellValue(i, "CCNumber");
				exMonth = TableContainer.getCellValue(i, "exMonth");
				exYear = TableContainer.getCellValue(i, "exYear");
				securityCode = TableContainer.getCellValue(i, "securityCode");
			...
			testStatus = verifyXPath.isfound(Config.driver, pr.getProperty("CHECKOUT_ERR_MSG_XPATH"));
			StatusLog.printlnPassedResultTrue(Config.driver,"[TESTCASE] Error Message is display after order payment is declined.",testStatus);
